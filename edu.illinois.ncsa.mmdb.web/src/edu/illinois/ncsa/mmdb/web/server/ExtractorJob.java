...
public class ExtractorJob implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            rd.close();
            //remove cached previews if we've succeeded in creating new ones
            log.debug("Removing cached previews of :" + uri);
            TupeloStore.getInstance().removeCachedPreview(uri, GetPreviews.SMALL);
            TupeloStore.getInstance().removeCachedPreview(uri, GetPreviews.LARGE);
