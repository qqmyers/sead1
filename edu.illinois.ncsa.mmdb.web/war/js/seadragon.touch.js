...
//Attribute
var view_w = view_width = 600; //View port
var view_h = view_height = 600;
var image_width = 0;
var image_height = 0;
var image_left = 0;
var image_top = 0;
...
function SDT_enable_touch(elem) {
	if (touch) {
		elem.hammer({swipe_time:300, prevent_default: false, hold:false, tap_double:false, scale_treshold: 0, drag_min_distance: 0});
...
function do_dragend(event) {
	if(turning_lock) {
		event.type = "touchend";
		magazine.trigger(event);
		setTimeout(function() {dMagazine.css("z-index",-100); /*dBase.show(); dOverlay.show();*/},2000);
	}
	else {
		do_release(event);
	}
