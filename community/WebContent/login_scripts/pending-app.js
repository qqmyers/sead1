...
seadSpaces.doPendingAjax = function(url){
	return $.ajax({
		type : "GET",
		timeout : '10000',
		url : url + "/resteasy/collections/metadata/http%3A%2F%2Fsead-data.net%2Fterms%2FProposedForPublication/literal/true",
		dataType : "json"
	});
}
...
seadSpaces.buildPendingGrid = function(size, i, projectName, projectDescription, projectLogo, projectColor, projectBg, value, pendingRequests){
...
$.each(pendingRequests, function () {
	page += '<li class="collection">'+'<a href=\"'+this.url + '\">' +  this.Collection + '</a></li>';
});
page += '<ul>';
}
...
seadSpaces.init = function(){
	...
	$.each( spaces, function( key, value ) {  	
  	$.when(seadSpaces.doConfigAjax(value), seadSpaces.doPendingAjax(value)).then(function(config, requests){
var pendingRequests=[];

$.each(requests[0], function(item) {

if(item != "@context") {

pendingRequests.push({"Collection":requests[0][item].Title, "url":value + "#collection?uri="+requests[0][item].Identifier});
}
});
		 ...
		 if(typeof projectLogo !== 'undefined' && projectLogo.substring(0, 6) == "images"){
		    projectLogo  = value+'/'+projectLogo;
         	...
         	seadSpaces.buildPendingGrid(size,i,projectName,projectDescription,projectLogo,projectColor,projectBg, value, pendingRequests);
         	i++;
         }).fail(function (response) {
        	 	seadSpaces.buildPendingGrid(size,i,null,null,null,null,null,value, null);
        	 	i++;
    });
