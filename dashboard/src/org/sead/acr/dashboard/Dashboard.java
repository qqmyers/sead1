...
public class Dashboard extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			String collections = mp.getJSONResponse(Queries.ALL_TOPLEVEL_COLLECTIONS);
				...
				for(int i=0; i< resultArray.length(); i++) {
					try {
						...
						if(nameParts.length==2) {
							...
							String mimeType = org.sead.acr.common.utilities.MimeMap.findCategory(fileExt);
						...
						//Add filenames with no extension as unknown mime types? Use mimetype info in Medici rather than re-deriving from file extension?
